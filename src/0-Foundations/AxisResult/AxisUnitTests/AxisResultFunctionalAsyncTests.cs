...
public class AxisResultFunctionalAsyncTests
{
    ...
    {
        var called = false;
        var r = await AxisResult.Ok(5).TapAsync(() => { called = true; return Task.CompletedTask; });
        Assert.True(called);
        Assert.Equal(5, r.Value);
    }
    ...
    {
        var r = await AxisResult.Error<int>(E1).MapErrorAsync(
            _ => Task.FromResult<IEnumerable<AxisError>>([E2]));
        Assert.Equal("E2", r.Errors[0].Code);
    }
    ...
    #region ThenUnlessAsync

    [Fact]
    public async Task ThenUnlessAsync_PredicateTrue_Skips_Next_And_Preserves_Value()
    {
        var nextCalled = false;
        var r = await AxisResult.Ok(10).ThenUnlessAsync(x => x > 0, _ => { nextCalled = true; return Task.FromResult(AxisResult.Ok()); });
        Assert.True(r.IsSuccess);
        Assert.Equal(10, r.Value);
    ...
    [Fact]
    public async Task ThenUnlessAsync_PredicateFalse_Next_Succeeds_Preserves_Value()
    {
        var nextCalled = false;
        var r = await AxisResult.Ok(10).ThenUnlessAsync(x => x < 0, _ => { nextCalled = true; return Task.FromResult(AxisResult.Ok()); });
        Assert.True(r.IsSuccess);
    ...
    [Fact]
    public async Task ThenUnlessAsync_PredicateFalse_Next_Fails_Propagates_Error()
    {
        var r = await AxisResult.Ok(10).ThenUnlessAsync(x => x < 0, _ => Task.FromResult(AxisResult.Error(E1)));
        Assert.True(r.IsFailure);
        Assert.Equal("E1", r.Errors[0].Code);
    }

    [Fact]
    public async Task ThenUnlessAsync_Already_Failure_Skips_Predicate_And_Next()
    {
        var predicateCalled = false;
        var nextCalled = false;
        var r = await AxisResult.Error<int>(E2).ThenUnlessAsync(
            _ => { predicateCalled = true; return true; },
            _ => { nextCalled = true; return Task.FromResult(AxisResult.Ok()); });
        Assert.True(r.IsFailure);
        Assert.Equal("E2", r.Errors[0].Code);
    ...
    #region ThenWhenAsync

    [Fact]
    public async Task ThenWhenAsync_PredicateFalse_Skips_Next_And_Preserves_Value()
    {
        var nextCalled = false;
        var r = await AxisResult.Ok(10).ThenWhenAsync(x => x < 0, x => { nextCalled = true; return Task.FromResult(AxisResult.Ok(x * 2)); });
        Assert.True(r.IsSuccess);
        Assert.Equal(10, r.Value);
        Assert.False(nextCalled);
    }
    ...
    [Fact]
    public async Task ThenWhenAsync_PredicateTrue_Next_Succeeds_Replaces_Value()
    {
        var r = await AxisResult.Ok(10).ThenWhenAsync(x => x > 0, x => Task.FromResult(AxisResult.Ok(x * 2)));
        Assert.True(r.IsSuccess);
        Assert.Equal(20, r.Value);
    }

    [Fact]
    public async Task ThenWhenAsync_PredicateTrue_Next_Fails_Propagates_Error()
    {
        var r = await AxisResult.Ok(10).ThenWhenAsync(x => x > 0, _ => Task.FromResult(AxisResult.Error<int>(E1)));
        Assert.True(r.IsFailure);
        Assert.Equal("E1", r.Errors[0].Code);
    }

    [Fact]
    public async Task ThenWhenAsync_Already_Failure_Skips_Predicate_And_Next()
    {
        var predicateCalled = false;
        var nextCalled = false;
        var r = await AxisResult.Error<int>(E2).ThenWhenAsync(
            _ => { predicateCalled = true; return true; },
            x => { nextCalled = true; return Task.FromResult(AxisResult.Ok(x)); });
        Assert.True(r.IsFailure);
        Assert.Equal("E2", r.Errors[0].Code);
        Assert.False(predicateCalled);
        Assert.False(nextCalled);
    ...
    [Fact]
    public async Task RecoverConflictAsync_AllConflict()
    {
        var r = await AxisResult.Error<int>([AxisError.Conflict("C1"), AxisError.Conflict("C2")]).RecoverConflictAsync(() => Task.FromResult(42));
        Assert.Equal(42, r.Value);
    }

    [Fact]
    public async Task RecoverConflictAsync_Mixed()
    {
        var r = await AxisResult.Error<int>([AxisError.Conflict("C1"), E2]).RecoverConflictAsync(() => Task.FromResult(42));
        Assert.True(r.IsFailure);
    }

    [Fact]
    public async Task RecoverConflictAsync_ResultRecovery()
    {
        var r = await AxisResult.Error<int>(AxisError.Conflict("C1")).RecoverConflictAsync(() => Task.FromResult(AxisResult.Ok(7)));
        Assert.Equal(7, r.Value);
    }
    ...
    [Fact]
    public async Task ElseNotFoundAsync_Success_MapsValue()
    {
        var r = await AxisResult.Ok(21).ElseNotFoundAsync(x => Task.FromResult($"value:{x}"), () => Task.FromResult("default"));
        Assert.Equal("value:21", r.Value);
    }

    [Fact]
    public async Task ElseNotFoundAsync_AllNotFound_UsesRecovery()
    {
        var r = await AxisResult.Error<int>([E1, AxisError.NotFound("X")]).ElseNotFoundAsync(x => Task.FromResult($"value:{x}"), () => Task.FromResult("default"));
        Assert.Equal("default", r.Value);
    }

    [Fact]
    public async Task ElseNotFoundAsync_OtherError_PropagatesErrors()
    {
        var r = await AxisResult.Error<int>(E2).ElseNotFoundAsync(x => Task.FromResult($"value:{x}"), () => Task.FromResult("default"));
        Assert.True(r.IsFailure);
        Assert.Equal("E2", r.Errors[0].Code);
    }
