...
// Exercises the dialect-agnostic IAxisSagaSettingsStore against a real Postgres AXIS_SAGA.SAGA_SETTINGS row.
// Each test controls the single settings row through InitializeAsync (set to the seeded default) and leaves
// it unbounded in DisposeAsync so it can't gate the other tests that share this container's collection.
[Collection("AxisSagaPostgresCollection")]
public class AxisSagaSettingsStoreTests(AxisSagaPostgresFixture fixture) : IAsyncLifetime
{
