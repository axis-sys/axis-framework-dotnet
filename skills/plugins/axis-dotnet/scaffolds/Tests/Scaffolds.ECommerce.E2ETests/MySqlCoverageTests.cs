...
// The single [Fact] holder for the MySql-pinned run — mirrors PostgresCoverageTests exactly, minus the
// [Fact(Skip)] guard, which flips with ECommerceProviderPin.DefaultProvider (testing-e2e-hermetic-provider-pinning).
[Collection(MySqlECommerceWebAppCollection.Name)]
public sealed class MySqlCoverageTests(MySqlECommerceWebAppFactory factory)
{
    private const string? Skip = ECommerceProviderPin.DefaultProvider == "MySql"
        ? null
